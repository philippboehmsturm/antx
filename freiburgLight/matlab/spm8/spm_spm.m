...
% $Id: spm_spm.m 669 2012-02-27 14:10:24Z vglauche $
...
try
    ...
    if isfield(xVi,'V')
        ...
        %------------------------------------------------------------------
        W     = spm_sqrtm(spm_inv(full(xVi.V)));
        W     = W.*(abs(W) > 1e-6);
        if issparse(xVi.V)
            xX.W  = sparse(W);
        else
...
if isfield(xX,'W')
    ...
    %-Initialise arfit trackorder image file(s)
    %----------------------------------------------------------------------
    if isfield(xVi,'arfit')
        if iscell(xVi.arfit.firstpass)
            fnames   = arrayfun(@(cs)sprintf('TrackOfirst_%02d.img',cs),1:numel(SPM.Sess),'UniformOutput',false);
            descrips = arrayfun(@(cs)sprintf('Track Order (1st pass) - Session %d',cs),1:numel(SPM.Sess),'UniformOutput',false);
            VTrackOfirst = struct('fname',    fnames,...
                'dim',      DIM',...
                'dt',       [spm_type('uint8') spm_platform('bigend')],...
                'mat',      M,...
                'pinfo',    [1 0 0]',...
                'descrip',  descrips);
            VTrackOfirst = spm_create_vol(VTrackOfirst);
        end
        fnames   = arrayfun(@(cs)sprintf('TrackOfinal_%02d.img',cs),1:numel(SPM.Sess),'UniformOutput',false);
        descrips = arrayfun(@(cs)sprintf('Track Order (final) - Session %d',cs),1:numel(SPM.Sess),'UniformOutput',false);
        VTrackOfinal = struct('fname',    fnames,...
            'dim',      DIM',...
            'dt',       [spm_type('uint8') spm_platform('bigend')],...
            'mat',      M,...
            'pinfo',    [1 0 0]',...
            'descrip',  descrips);
        VTrackOfinal = spm_create_vol(VTrackOfinal);
    end    
...
for z = 1:nbz:zdim                       %-loop over planes (2D or 3D data)
    ...
    for bch = 1:nbch                     %-loop over blocks
        ...
        if CrS
            ...
            %-If ReML/ARFIT hyperparameters are needed for xVi.V
            %--------------------------------------------------------------
            if ~isfield(xVi,'V')
                ...
                if ~isempty(j)
                    ...
                    Y  = Y(:,j)*q;
                    Cy = Cy + Y*Y';
                    if isfield(xVi, 'arfit') % may exist for fMRI designs
                        for k = 1:numel(SPM.Sess)
                            for l = 1:numel(j)
                                xVi.arfit.Sess{k} = arfit_update(xVi.arfit.Sess{k},res(SPM.Sess(k).row,j(l)));
                                xVi.arfit.Sess{k} = arfit_order(xVi.arfit.Sess{k});
                            end
                        end
                    end
                end
    ...
    if isfield(xX,'W')
        ...
        if ~isempty(Q)
            if isfield(xVi,'arfit')
                if iscell(xVi.arfit.firstpass)
                    for k = 1:numel(SPM.Sess)
                        jj(Q) = xVi.arfit.firstpass{k}.trackorder((end-numel(Q)+1):end);
                        VTrackOfirst(k) = spm_write_plane(VTrackOfirst(k), jj, CrPl);
                    end
                end
                for k = 1:numel(SPM.Sess)
                    jj(Q) = xVi.arfit.Sess{k}.trackorder((end-numel(Q)+1):end);
                    VTrackOfinal(k) = spm_write_plane(VTrackOfinal(k), jj, CrPl);
                end
            end
        end
...
if ~isfield(xVi,'V')
    ...
    if isfield(xVi,'arfit')
        if ~isfield(xVi.arfit,'firstpass')
            %-1st pass - estimate arfit order
            %------------------------------------------------------------------
            for k = 1:numel(xX.K)
                % Get estimated order, save first pass fit
                % xVi.arfit.firstpass{k} = arfit_order(xVi.arfit.Sess{k}); % Currently not needed, arfit_order called for each bunch of data.
                xVi.arfit.firstpass{k} = xVi.arfit.Sess{k};
                % Initialise new arfit
                xVi.arfit.Sess{k} = arfit_init(xVi.arfit.firstpass{k}.estimated_order);
            end
        else
            %-2nd pass - Get ACF from arfit
            %------------------------------------------------------------------
            V = cell(size(xX.K)); % sparse is inefficient during inversion/sqrtm
            for k = 1:numel(xX.K)
                % need to make acf expansion length configurable, currently
                % fixed to 32 sec
                acfexp = min(ceil(xVi.arfit.exp/SPM.xY.RT),nnz(xX.K(k).row)-1);
                acf = sparse(1,1:(acfexp+1),squeeze(arfit_acf(xVi.arfit.Sess{k},acfexp)),1,nnz(xX.K(k).row));
                V{k} = toeplitz(acf);
            end
            xVi.V = blkdiag(V{:});
        end
    ...
    if ~isfield(xX,'W')
        szSPM = whos('SPM');
        if (spm_check_version('matlab','7') >=0) && (szSPM.bytes < 2*(1024^3))
            save('SPM.mat','SPM','-V6');
        else
            save('SPM.mat','SPM');
        end
...
%==========================================================================
xX.V            = spm_filter(xX.K,spm_filter(xX.K,full(W*V*W'))');% KWVW'K'
...
if isfield(xVi,'arfit')
    if iscell(xVi.arfit.firstpass)
        SPM.VTrackOfirst = VTrackOfirst;
    end
    SPM.VTrackOfinal = VTrackOfinal;
end
