function rfx_display_con_table(cons,opts,xyzstr)
%
% display a table with subject directories for group specification and
% exclusion of subjects
% -------------------------------------------------------------------------
% $Id: rfx_display_con_table.m 13 2010-07-15 08:38:20Z volkmar $
...
try
  ...
  while f <= length(cons)
    ...
    text(dx2,tabentry,'contrast name','HorizontalAlignment','left');
    y = y - dy/2;
    ...
    while f <= length(cons)
      ...
      text(dx2+0.01,y,sprintf('%s',cons{f}),'HorizontalAlignment','left');
      y = y - dy;
      f = f + 1;
      if y < dy
        break
