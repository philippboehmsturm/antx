...
switch cmd
    case 'init'     %function addquiver(handle, Vqfnames, Vmaskfname, varargin)
        ...
        if length(Vq) == 3
            st.vols{volhandle}.quiver = struct('qx',Vq(1),'qy',Vq(2),'qz',Vq(3), ...
                ...
                'qst',3,'ql',.9,'qht',[],'qhc',[],'qhs',[], 'qw', .5, 'ref',[]);
        else
            error('spm_orthviews(''quiver'', ''init'',...): Please specify 3 images!');
        end;
        ud = dti_get_dtidata(Vq(1).fname);
        st.vols{volhandle}.quiver.ref = ud.ref;
        if nargin > 4
    ...
    case 'redraw'
        ...
        if isfield(st.vols{volhandle},'quiver')
            ...
            Mr   = diag([-1 1 1])*st.Space(1:3,1:3)*st.vols{volhandle}.premul(1:3,1:3)*st.vols{volhandle}.quiver.ref(1:3,1:3);
            if ~isempty(fa)
            ...
            rqt = Mr*reshape(rqt,TD(1)*TD(2),3)';
            ...
            rqc = Mr*reshape(rqc,CD(1)*CD(2),3)';
            ...
            rqs = Mr*reshape(rqs,SD(1)*SD(2),3)';
            ...
            if exist('spm_orthviews/tbxdti_quiver3') == 2
                quiverfun = @tbxdti_quiver3;
            else % fallback
                quiverfun = @quiver3;
            ...
            st.vols{volhandle}.quiver.qht = feval(quiverfun,xtt,ytt,z,u,v,w,ql,ls,'Parent',st.vols{volhandle}.ax{1}.ax);
            ...
            xtt=repmat(x,size(z,1),1);
            ytt=repmat(y',1,size(z,2));
            st.vols{volhandle}.quiver.qhc = feval(quiverfun,xtt,ytt,z,u,v,w,ql,ls,'Parent',st.vols{volhandle}.ax{2}.ax);
            ...
            xtt=repmat(x,size(z,1),1);
            ytt=repmat(y',1,size(z,2));
            st.vols{volhandle}.quiver.qhs = feval(quiverfun,xtt,ytt,z,u,v,w,ql,ls,'Parent',st.vols{volhandle}.ax{3}.ax);
    ...
    case 'context_edit'
        ...
        spm_input('!DeleteInputObj',Finter);
        st.vols{volhandle}.quiver.(varargin{3}) = in;
        spm_orthviews('redraw');
