function out = FAVBS_do_the_job(rVb0, rVFA, rVmean, FA_on, iter, VGstemp)
...
if dotemplate
    if(FA_on==0)
        ...
        if(FA_on==1)
            ...
            VBMSK_average = find_mask_meanDWI(VGmean); % ->HIER FEHLER!!!
...
while(procent_diff>2 && ddiff>2),
    ...
    switch FA_on
        case 0
            matnames = norm_spm_FAVBS(VG,rVb0,'',FA_on,iter);
        case 1
            matnames = norm_spm_FAVBS(VG,rVFA,rVmean,FA_on,iter);
        case 2
            [matnames, fmatnames] = norm_spm_FAVBS(VG,rVFA,rVmean,FA_on,iter);
    end
