...
% Volkmar Glauche
% $Id: cfg_run_favbs_create_template.m 689 2009-11-12 10:58:57Z glauche $
      ...
      % Begin: do the job   
            ...
            if(strcmp('CT',fieldnames(job.norm.templ))) % Customized Template (CT) Generation
                if(strcmp('SCT',fieldnames(job.norm.templ.CT))) % Symmetrisized CT
                    out = FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on,iter,1);
                elseif(strcmp('NCT',fieldnames(job.norm.templ.CT))) % Normal CT
                    out = FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on,iter,0);
                end
            elseif(strcmp('ET',fieldnames(job.norm.templ))) % External Template (ET) 
                ...
                out = FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on, iter, VG);
            end
