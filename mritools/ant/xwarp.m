...
function [success]=xwarp(pstruct)
...
%         [hb a]=rgetnii(fullfile(pa, '_test1.nii'));
    ...
    if 1  %TESTS ELASTIX
        %% TESTBACKWARD
        doelastix('transforminv'  , pa,    {fullfile(pa,'AVGT.nii')}  ,3,M )
%         doelastix('transforminv'  , pa,    {fullfile(pa,'_tpmgrey.nii')}  ,-1,M )
    ...
    if 1
        xdeform2(fullfile(pa,'AVGT.nii'),-1, [nan nan nan],4 )
        xdeform2(fullfile(pa,'t2.nii'),        1, [nan nan nan],4 )
    end
