...
if allsubjects==1
    pas=an.mdirs;
    files     =spm_select('FPListRec',an.datpath,'.*.nii');
else
    pas=antcb('getsubjects') ;
    files={};
    for i=1:length(pas)
        fdum     =spm_select('FPListRec',pas{i},'.*.nii');
        files=[files;fdum];
    end
end


filesuni=unique(regexprep(files,['.*' filesep filesep],''));
%sort oddEven
...
set(gca,'ytick',[]);
cases=regexprep(pas,['.*' filesep filesep ''],'');
set(gca,'xtick',[1: size(ma,2)], 'xticklabels',cases,'fontweight','bold');

try; xticklabel_rotate([],55,cases,'HorizontalAlignment','right','fontsize',6); end
