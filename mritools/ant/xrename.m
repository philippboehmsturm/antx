...
% #yg GUI-USAGE
% #g RENAME: in the [NEWNAME]-COLUMN type a new name (but not "##" (without ""!), since this would delete the file)
%     - the [EXTRACT VOLNUM]-column must be empty
%     --> RENAMING REALLY MEANS renaming this file
% #g COPY: in the [NEWNAME]-COLUMN type a file-name (but not "##" (without ""!), since this would delete the file)
%     - the [EXTRACT VOLNUM]-column must contain ":"  (the ":" means take all containing volums)
%     --> the ":"  tag without "", dont forget the ":", otherwise the original file will be renamed 
%     --> in this case a copy of the original file is created 
% #g DELETE: in the [NEWNAME]-COLUMN type a "##"
%    - the [EXTRACT VOLNUM]-column must be empty
%    --> DELETION REALLY MEANS DELTING this file !!
% #g EXTRACT: new filename in the [NEWNAME]-column must be given (but not '##'  ..this would delete the orig. file)  
...
% #g EXPAND: new filename in the [NEWNAME]-column must be given (but not '##'  ..this would delete the orig. file)  
...
%      xrename                                           %open gui
%      xrename(1);                                       %open gui
%      xrename(1, 'mist5.nii' , 'mist7.nii' )            %open gui, preselect: 'mist5.nii' should be renamed to  'mist7.nii'
%      xrename(1,{'mist5.nii'},{'mist7.nii'})            %same as above
%      xrename(1,{'mist5.nii' 'mist6.nii'},{'mist7.nii' 'mist8.nii'})    %open gui, preselect: 'mist5.nii' should be renamed to  'mist7.nii', and 'mist6.nii' renamed to  'mist8.nii'
%      xrename(1,{'mist5.nii' 'mist6.nii'}',{'mist7.nii' 'mist8.nii'}')  %same..
%      xrename(0,{'mist5.nii' 'mist6.nii'}',{'mist7.nii' 'mist8.nii'}')  %same,but without opening GUI
%
% #g COPYING-EXAMPLE
...
% % ••••••••••••••••••••••••••••••••••••••••••••••••••••••
% % BATCH:        [xrename.m]                             
% % descr:  rename file(s) from selected ant-mousefolders 
% % ••••••••••••••••••••••••••••••••••••••••••••••••••••••
% z.files=[];                                             
% z.files={ 'mist.nii' 	     '##'               %% this file is deleted                    
%           'mist5.nii' 	'mist10.nii'        %% 'mist5.nii'  is renamed to 'mist10.nii'                   
%           'mist6.nii' 	'mist11' };         %% 'mist6.nii'  is renamed to 'mist11.nii'  (NOTE: 2nd column,i.e. the new name list, does not need file extension)                      
...
%———————————————————————————————————————————————
 if 0
     ...
     xrename(1)
     xrename(1, 'mist5.nii' , 'mist7.nii' )
     xrename(1,{'mist5.nii'},{'mist7.nii'})
     xrename(1,{'mist5.nii' 'mist6.nii'},{'mist7.nii' 'mist8.nii'})
     xrename(1,{'mist5.nii' 'mist6.nii'}',{'mist7.nii' 'mist8.nii'}')
     xrename(0,{'mist5.nii' 'mist6.nii'}',{'mist7.nii' 'mist8.nii'}')
 end
...
for i=1:size(pa,1)      %PATH
    for j=1:length(fi)  %FILE
        ...
        if exist(s1)==2  %CHK EXISTENCE
            if strcmp(finew{j},'##')==1  % DELETE FILE
%                 if 1
%                     disp('---delete-test--');
%                     disp(['del: ' s1]);
%                 end
