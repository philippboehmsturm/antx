...
%% pre-select folders by either file/id/folder or textfile
% xselect(showgui,x)
%% #by PARAMETERS
% showgui: opens the gui [0,1]
% x      : struct with following parameters
%  #g   x.files  if searching for one or more file within folders
%  #g   x.logop       : to define whether a folder contains all or one of the files defined in x.files
%                     -x.logop is only necessary if x.files is used          
%    ---alternative---
%  #g   x.ids         : array of ids  , example [1 ,30,31] -->select indices 1,30 and 31 in listbox
%                   -use with care (new mouse cases/folders will change the order in the listbox!)
%    ---alternative---
...
%     
%% #by DESCR:
% -%these folders will be pre-selected in the ant-gui-listbox
% -->BATCH: possible
%% #by OPTIONAL INPUT:
...
% xselect(1,struct('files',{{'AVGT.nii';'hemi.nii',}},'logop','&'))  % #g select folder, with existing files AVGT.nii AND hemi.nii  
% xselect(1,struct('files',{{'AVGT.nii';'hemi.nii',}},'logop','|'))  % #g select folder, with existing files AVGT.nii OR hemi.nii  
...
p={...
    ...
    'inf3'      '   -folders must contain a specified "filter"-file' '' ''
    'inf4'      '   -if multiple filter-files selected, one has to define whether..' '' '' 
    'inf5'      '     a folder contains (a) ALL filter-files (logical operator: "&") '  '' ''
    'inf6'      '                    or (b) ONE of the filter-files (logical oeprator: "|") '  '' ''     
    ...
    'logop'     '&'           'OPTION-1: LOGICAL OPERATOR (FOR MULTIPLE-FILTER-FILES ONLY) TO define selected folders ("&" ...and "|"...or ) for FOLDERS to find'  {'& ..all files has to be found in folder'; '|..one of the files has to be found in folder' }
